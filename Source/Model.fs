...
type Model () as model = 
    ...
    let mutable servo =        lazyPropertyInit model.ServoConfig        (fun x -> new Devices.ServoMotor(x))
    let mutable motor =        lazyPropertyInit model.MotorConfig        (fun (cnum:int) -> new Devices.PowerMotor(cnum))
    let mutable encoder =      lazyPropertyInit model.EncoderConfig      (fun cnum -> new Sensors.Encoder(cnum))
    let mutable analogSensor = lazyPropertyInit model.AnalogSensorConfig (fun (cnum:int) -> new Sensors.AnalogSensor(cnum))
    ...
    member val ServoConfig = 
        [| (E1, ("/sys/class/pwm/ehrpwm.1:1", Defaults.Servo3 ))
           ...
           (C3, ("/sys/class/pwm/ecap.2",     Defaults.Servo4 )) |] with get, set
    member val EncoderConfig =
        [| (B1, 0x30)
           (B2, 0x31)
           (B3, 0x32)
           (B4, 0x33) |] with get, set
    member val MotorConfig = 
        [| (M1, 0x14)
           (M2, 0x15)
           (M3, 0x17)
           (M4, 0x16) |] with get, set
