...
module Observable =
    ...
    let interval1(timeSpan: System.TimeSpan) = 
        let counter = ref 0
    ...
    let distinctUntilChanged(source: IObservable<'T> when 'T: equality) = 
        let hasCurrentKey = ref false
        let currentKey = ref Unchecked.defaultof<'T>
        let notifier = new Subject<'T>()
